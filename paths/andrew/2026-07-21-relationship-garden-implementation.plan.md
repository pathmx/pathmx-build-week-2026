...
# Relationship Garden — Implementation Plan For Agents

Handoff plan for another agent to implement the first Relationship Garden lab.
Product intent lives in
[2026-07-21-relationship-garden-outline.notes.md](./2026-07-21-relationship-garden-outline.notes.md).
...
**Owner intent (Andrew):** strategy notes live under `paths/andrew/` on `main`.
Implement the playable lab under `paths/labs/relationship-garden/`. Do link
from `paths/labs/index.path.md` when the lab is playable. Do not invent PathMX
plugins or Host Actions.
...
| Topic | Decision |
| --- | --- |
| Lab id / route | `relationship-garden` → `/labs/relationship-garden` |
| Display name | **Relationship Garden** in titles; in-UI microcopy prefers “your garden,” “tend,” “dormant,” never “leads” or “pipeline” |
...
1. Open `/labs/relationship-garden` in Play.
...
```text
paths/labs/relationship-garden/
├── index.demo.md                 # playable tending path
├── relationship-garden.components.md
├── relationship-garden.css         # optional; prefer scoped component CSS first
...
```json
{
  "generatedFor": "relationship-garden-mvp",
  "asOf": "2026-07-21",
  "people": [
    {
      "id": "priya-nair",
...
File: `relationship-garden.components.md`

### Components

1. `<relationship-garden>` — main instrument
...
Type: demo/workshop. Route: `/labs/relationship-garden`.
...
| 3 | `garden-glance` | Mount `<relationship-garden>` |
...
1. **Begin**
   `Help me tend my Relationship Garden for the internship bed. Read only paths/labs/relationship-garden/people/ and garden.people.json. Suggest one human to contact and why, using written notes only.`
2. **Capture**
...
- Author `<relationship-garden>` with bed filters + plant buttons + panel
- Wire JSON resource
...
- `/labs/relationship-garden` loads
...
- Task title suggestion: `Build Relationship Garden MVP lab`
...
- [ ] Lab exists at `paths/labs/relationship-garden/` with route working
